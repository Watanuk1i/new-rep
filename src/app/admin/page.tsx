...
function FullResetSection() {
  ...
  const run = async () => {
    ...
    try {
      ...
      try { await sb.from('debt_game_actions').delete().neq('id', ''); } catch {}
      try { await sb.from('debt_games').delete().neq('id', ''); } catch {}
      await sb.from('debts').delete().neq('id', '');
      await sb.from('challenges').delete().neq('id', '');
      ...
      const seed: Array<[string, string, string, number, number, boolean]> = [
        ...
        // Активный Макото и Маки — по запросу владельца
        ['p-1',          'Макото Наэги',             'player',   1_000_000,   30,  true],
        ['p-maki',       'Маки Харукава',            'player',   1_000_000,   20,  true],
