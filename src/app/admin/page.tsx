...
function AccountsTab() {
  ...
  // Сортируем: GM сверху, потом Селестия, потом игроки
    ...
    <div className="space-y-3">
      <div className="glass-strong gold-border p-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          Для GM — <span className="font-mono text-gold">host</span>, для Селестии — <span className="font-mono text-gold">queen</span>,
          для остальных — имя персонажа целиком (например, <span className="font-mono text-gold">Макото Наэги</span>).
        </p>
      ...
      <div className="glass p-3 text-[11px] text-muted-foreground">
        💡 Если логин <span className="font-mono text-gold">host</span> / <span className="font-mono text-gold">queen</span> не работает —
        значит в БД сидируются неправильные id (например UUID-ы от старого seed). Запусти{' '}
        <span className="font-mono text-gold">supabase/fix_accounts.sql</span> в Supabase SQL Editor.
      </div>
    </div>
  );
}
