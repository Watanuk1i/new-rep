...
export default function SuperGameDetailPage() {
  ...
  const isCardShip = game.type === 'card_ship';
  ...
  return (
    <div className="px-3 sm:px-4 py-4 max-w-2xl mx-auto space-y-4 animate-fade-in">
      {/* Для Карточного корабля основной вид — это CardShipBoard.
          Карточка с описанием/правилами/составом схлопнута в expandable. */}
      {isCardShip ? (
        <>
          <CardShipBoard superGame={game} />

          {/* Дополнительно — кнопка перевести йены прямо из игры */}
          {currentUser && currentUser.status !== 'gm' && (
            <>
              ...
              >
                💸 Перевести йены игроку (вне рынка, по договорённости)
              </button>
              <TransferModal
                open={transferOpen}
                onClose={() => setTransferOpen(false)}
                relatedGameId={game.id}
                defaultComment="Сделка в Карточном корабле"
              />
            </>
          )}

          {/* Правила, схлопываемые */}
          {game.rules && (
            <details className="glass p-4">
              <summary className="cursor-pointer text-sm font-bold text-gold">
                📋 Правила
              </summary>
              <p className="text-sm whitespace-pre-line text-muted-foreground mt-2">
                {game.rules}
              </p>
            </details>
          )}
        ...
        <>
          {/* Старый, общий вид для прочих типов Больших игр */}
