...
const GAME_TYPES = [
  ...
  { type: 'throne_celestia', label: 'Трон Селестии', desc: 'Финал сезона. Селестия vs Претендент', count: 'все', rules: 'Финальная супер-игра. Селестия (p-queen) против Претендента, остальные выбирают сторону или нейтралитет.\nСтавки: Селестия — должность + 5M; Претендент — 2M (или весь баланс).\n10 раундов карт Император / Гражданин / Питомец (на первые 5 у Селестии Император, у Претендента Питомец; во вторых 5 стороны меняются).\nЕсли ничья после 10 раундов — «Последний трон» (только Император vs Питомец).\nПривилегия Селестии «Королевский регламент» (1 раз). Преимущества из фондов сторон: посмотреть карту 500k, заменить карту 700k, переиграть проигранный раунд 1M, заблокировать привилегию (только Претендент) 1.5M.\nПри победе Селестии — должность сохраняется. При победе Претендента — выбор: Новый директор или Бунт победил. Финальные последствия — ручные кнопки ведущего.', isLive: true },
  ...
  { type: 'mini_joker',      label: 'Малая · Достать Джокера', desc: '3 режима · 2–6 игроков', count: '2-6', rules: '3 режима:\n— Быстрый: первый, кто вытянул Джокера, проигрывает; остальные делят банк после комиссии 5%.\n— Долгий: вытянувший Джокера выбывает, колода обновляется (10+1), играют до одного.\n— Расширенный: как долгий + действия: пропустить ход 20k (1 раз за игру), купить подсказку 30k (показывает шанс и уровень риска), передать ход (получатель принимает или отказывается).\nКолода: 10 обычных + 1 Джокер. Шанс публично показывается всем.', isLive: true },
