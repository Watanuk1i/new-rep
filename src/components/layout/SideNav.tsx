...
// Боковая навигация для десктопа (lg+).
// Корректное responsive: на мобильных скрыта (используется TopBar drawer + BottomNav).
...
const ITEMS = [
  { href: '/account', icon: '💳', label: 'Аккаунт и баланс' },
...
export function SideNav() {
  ...
  return (
    <aside className="hidden lg:block w-64 shrink-0 sticky top-20 self-start max-h-[calc(100vh-6rem)] overflow-y-auto py-6 pr-2">
      ...
      {currentUser && (
        <Link href="/account" className="glass p-3 flex items-center gap-3 mb-4">
      ...
      <nav className="space-y-1">
        ...
        {!currentUser && (
          <Link
            href="/login"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gold hover:bg-gold/10 mt-2"
          >
            <span>➜</span>
            <span>Войти</span>
          </Link>
