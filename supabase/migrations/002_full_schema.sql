...
-- ========== SEED 16 участников ==========
-- ВАЖНО: 9 колонок -> по 9 значений в каждой строке.
...
-- Без этого свежесозданные таблицы будут видны как
-- "Could not find the table 'public.<name>' in the schema cache"
-- до тех пор пока PostgREST не перечитает схему сам (раз в минуту).
